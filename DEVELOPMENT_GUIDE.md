...
### 10.31 主线转向 QQ 官方平台：砍掉 LLBot，以及为什么用户数据迁不了

QQ 号被风控卡死之后，最终决定不再和 LLBot/NapCat 这条线纠缠，
**另开一个仓库直接对接 QQ 官方开放平台**（`KirikoBot-QQBot`）。
这一段记的是迁移本身的判断，不是平台 API 的实测（那份在
`docs/phase0-findings.md`，代码里的长期结论在 `docs/platform-migration.md`）。

#### 1. 为什么不做"传输层抽象"，而是直接砍功能

第一版尝试写一个能同时跑 OneBot 和官方平台的传输层。**写完又删了。**

抽象的收益来自"有多个等价的真实实现"。但这两套的能力集合不是"交集 + 各自特例"，
而是**官方平台严格更小**：没有主动推送、默认收不到非 @ 群消息、没有语音合成、不能 @ 群成员。
适配层要为 100% 的调用点准备两条路径，而其中一条永远是"不支持"。

更关键的是：那些不支持的能力，在 OneBot 侧是**一整批功能的根基**——
群语境、群统计、聊天回看、定时播报、提醒，全都建立在"能看到群里所有消息"和
"能主动往群里发消息"这两件事上。留着一个"能编译但永远失败"的分支，
只会让后来的人以为它能用。

所以最终方案是**直接面向官方平台重写接入层**（`qq_official.py` + `qq_gateway.py`），
把用不上的功能整块删掉，删掉的理由逐条写进 README。
`transport/` 目录从创建到删除就是这一段的产物。

> **教训**：不要为了"保留可能性"而抽象。**没有第二个真实实现的接口，是负债不是资产。**
> 判断标准很简单——如果第二个实现注定有一半方法是 `raise NotImplementedError`，
> 那就不是抽象，是把一个不存在的问题建模出来。

#### 2. ID 空间：为什么用户历史数据一律迁不了
...
实测同一个用户在同一 AppID 下，事件里的 `id`、`member_openid`、`union_openid`
是**同一个值**；`union_user_account` 字段根本不存在。

`user_openid` 按 AppID 分发这一点是致命的：**换 AppID 就全部作废**。
旧库里所有 `user_id` / `group_id` 都是 QQ 号，和新 ID 空间没有任何可计算的映射关系。

所以 `tools/migrate_data.py` 只搬"和具体是谁无关"的参考数据
（塔罗牌义、箱头资料、版本日志、表情包索引），
并且**把不迁的表和原因逐条打印出来**——不打印的话，下次看的人会以为是脚本漏了。

#### 3. 一个具体的技术坑：消息 id 不是数字

官方消息 id 是 ~120 字符的字符串，形如 `ROBOT1.0_xxx.yyy!zzz`，带 `.` 和 `!`。
旧库把 `message_id` 声明成 `INTEGER`，读取路径里还有 `int(message_id)` 强转：

```python
try:
    mid = int(message_id)      # 官方 id 在这里直接 ValueError
except (TypeError, ValueError):
    return None                # 然后被静默当成"没找到被引用的消息"
```

`except` 把异常吞成了 `None`，所以症状不是报错，而是**引用感知无声失效**——
引用了消息，但机器人完全不知道。这类"吞掉异常导致功能静默降级"的写法，
在换 ID 空间的时候最容易踩：类型不匹配不报错，只是永远查不到。

#### 4. 部署形态变简单了

官方平台不需要在本地跑一个登录着的 QQ 客户端，所以 compose 里的 `pmhq`、`llbot`
两个服务**整个消失**，`qq_volume`（登录态设备卷）也不需要了——
扫码、设备锁、风控那一整套随之消失。整栈只剩一个 `my-robot`。

容器名用 `kirikobot-qq`，刻意和旧栈的 `kirikobot` 错开：两个 compose 项目同时在时，
同名容器会被对方 `up` 的时候抢走或重建（10.30 那个坑的另一面）。

`.env` 的 `ONEBOT_*` 换成 `QQ_APP_ID` / `QQ_APP_SECRET`；
入站 webhook 没了（改成 WebSocket 网关长连接），`webhook_auth.py` 一并删除。

#### 5. 迁完之后的形态

机器人从"在群里一直看着、偶尔插话、会定时播报"变成「**只有被 @ 时才存在**」。

降级但没删的是画像 / 好感度 / 自学习——它们还在，只是能观察到的样本
从"整群的发言"缩到"被 @ 的那些对话"，判断会明显变粗。

表情包库也冻结了：被动收集靠监听群消息，这条路径没了。
迁移时从旧部署把 1404 张全复制了过来（其中 648 张是当年运行时收集、从未进 git 的），
并把 `KirikoBot/stickers/` 加进 `.gitignore`——**已跟踪的 756 张仍然跟踪**
（否则新 clone 下来表情工具没东西可发），此后新增的一律不入库。
